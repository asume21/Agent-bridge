...
server.registerTool(
  "handoff.attach",
  {
    title: "Attach a note to a handoff",
    description: "Append a file/note reference to a handoff message.",
    inputSchema: z.object({
      id: z.string(),
  ...
  async ({ id, name, path: filePath, note, author }) => {
    const messages = await loadMessages();
    const message = messages.find((entry) => entry.id === id);
    if (!message) {
      throw new Error(`No handoff message found with id ${id}`);
    ...
    addHistory(message, createHistoryEntry(`Attachment added: ${name || filePath || "(unnamed)"}`, author));
    await saveMessages(messages);
...
server.registerTool(
  "handoff.analyze",
  {
    title: "Analyze multiple idea proposals",
    description: "Extract shared themes and unique directions from multiple ideas.",
    inputSchema: z.object({ ideas: z.array(z.object({ author: z.string().optional(), proposal: z.string().min(1), cons: z.array(z.string()).optional() })) }),
    outputSchema: z.object({ analysis: z.any() }),
  },
  async ({ ideas }) => {
    const analysis = analyzeIdeas(ideas);
    return {
...
server.setRequestHandler(async (request, responder) => {
  const transport = new StdioServerTransport();
  await server.connect(transport);
  await transport.handleRequest(request, responder);
});

const transport = new StdioServerTransport();
server.connect(transport);

console.log(`[handoff-bridge] handoff file: ${storagePath}`);

transport
  .listen()
  .then(() => console.log("[handoff-bridge] listening for MCP requests"))
  .catch((error) => {
    console.error("[handoff-bridge] failed to start:", error);
    process.exit(1);
  });
